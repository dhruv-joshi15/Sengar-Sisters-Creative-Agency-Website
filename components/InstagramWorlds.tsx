...
const icons=[UtensilsCrossed,Camera,Leaf,Sparkles];
...
function PostCard({post,featured=false,index=0}:{post:InstagramPost;featured?:boolean;index?:number}){
 return <a href={post.url} target="_blank" rel="noopener noreferrer" className={`world-post ${featured?'world-post-featured':''}`} aria-label={`View ${post.title} on Instagram (opens in a new tab)`}>
  ...
  {post.likes!==undefined&&post.likes>=1000&&<span className="world-post-engagement"><span><Heart size={13}/>{post.likes.toLocaleString('en-IN')} likes</span>{!!post.comments&&<span><MessageCircle size={13}/>{post.comments} comments</span>}</span>}
 </a>
}

function Voices({world}:{world:InstagramWorld}){return <div className="world-voices" aria-label={`Public audience reactions around ${world.handle}`}><span className="world-voices-label">A LITTLE LOVE FROM THE FEED ↘</span><div className="world-voices-grid">{world.comments.map((comment,i)=><a className="world-voice" href={comment.url} key={`${comment.handle}-${i}`} target="_blank" rel="noopener noreferrer" aria-label={`Read ${comment.handle}'s comment on its original Instagram post`} style={{animationDelay:`${-i*2.1}s`}}><span className="voice-index">0{i+1} / PUBLIC COMMENT</span><blockquote>“{comment.quote}”</blockquote><span className="voice-handle">{comment.handle}<ArrowUpRight size={13}/></span></a>)}</div></div>}
 ...
 return <section ref={ref} id="worlds" className="instagram-worlds container section" aria-labelledby="worlds-title">
  ...
  <div className={`world-stage mood-${world.id}`}>
   ...
   <AnimatePresence mode="wait" initial={false}><motion.div key={world.id} role="tabpanel" id="world-panel" aria-labelledby={`world-tab-${world.id}`} tabIndex={0} className="world-panel" initial={reduced?false:{opacity:0,y:12}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}} transition={{duration:reduced?0:.3,ease:[.22,1,.36,1]}}>
    ...
    <div className="world-side"><span className="world-side-label">A FEW MORE FAVOURITE FRAMES <span>↓</span></span><div className="world-side-posts">{world.posts.slice(1).map((post,i)=><PostCard post={post} key={post.id} index={i+1}/>)}</div></div>
