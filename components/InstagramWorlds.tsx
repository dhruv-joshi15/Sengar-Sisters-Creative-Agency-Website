...
import {ArrowUpRight,Play,Heart,MessageCircle,Camera,Leaf,Sparkles} from 'lucide-react';
import {instagramWorlds,instagramChecked,type InstagramPost,type InstagramWorld} from '@/data/instagram';
...
const icons=[Camera,Leaf,Sparkles];
...
export function InstagramWorlds(){
 const [selected,setSelected]=useState(1);const tabs=useRef<(HTMLButtonElement|null)[]>([]);const ref=useRef<HTMLElement>(null);const reduced=useMotionPreference();
 ...
 const key=(event:KeyboardEvent<HTMLButtonElement>,index:number)=>{let next=index;if(event.key==='ArrowRight')next=(index+1)%3;else if(event.key==='ArrowLeft')next=(index+2)%3;else if(event.key==='Home')next=0;else if(event.key==='End')next=2;else return;event.preventDefault();change(next);tabs.current[next]?.focus()};
 return <section ref={ref} id="worlds" className="instagram-worlds container section" aria-labelledby="worlds-title">
  <Reveal><Label number="02">THE LIVING PORTFOLIO</Label><div className="worlds-heading"><div id="worlds-title"><AnimatedHeading>Three worlds.<br/>One <em>creative eye.</em></AnimatedHeading></div><p>A brand’s voice. A table’s atmosphere. A wedding’s heartbeat.<br/>Different stories, the same instinct for a frame that makes you feel something.</p></div></Reveal>
  ...
  <div className={`world-stage mood-${world.id}`}>
   ...
   <AnimatePresence mode="wait" initial={false}><motion.div key={world.id} role="tabpanel" id="world-panel" aria-labelledby={`world-tab-${world.id}`} tabIndex={0} className="world-panel" initial={reduced?false:{opacity:0,y:12}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}} transition={{duration:reduced?0:.3,ease:[.22,1,.36,1]}}>
    <div className="world-story"><span className="world-eyebrow">{world.eyebrow}</span><h3>{world.title}<br/><em>{world.emphasis}</em></h3><p>{world.description}</p><div className="world-services">{world.services.map(service=><span key={service}>{service}</span>)}</div><ContactLink channel="instagram" href={world.url} className="world-profile" aria-label={`Explore ${world.handle} on Instagram`}><Image src={world.avatar} alt="" width={42} height={42}/><span><strong>{world.handle}</strong><small>STEP INTO THIS WORLD</small></span><ArrowUpRight size={18}/></ContactLink><p className="world-profile-note">{world.note}</p><span className="world-handnote" aria-hidden="true">A little more feeling. ✧</span></div>
    <motion.div className="world-feature" style={reduced?undefined:{y:featureY}}><PostCard post={world.posts[0]} featured/><span className="world-feature-seal" aria-hidden="true">THE<br/><em>{world.id==='food'?'good taste':world.id==='weddings'?'love story':'studio'}</em><br/>EDIT</span></motion.div>
  ...
  <div className="world-editorial-line"><span>THE FEELING BEHIND THE FRAME</span><em>{world.id==='food'?'Some stories are best tasted first.':world.id==='weddings'?'A thousand feelings. A fraction of a second.':'Make them pause. Give them a reason to stay.'}</em><span aria-hidden="true">✧</span></div><div className="world-footnote"><span><span className="world-live-dot"/>REAL COVERS. ORIGINAL STORIES.</span><p>Selected public posts & comments · Checked {instagramChecked}.<br/>Credits stay with the original creators. Engagement is a snapshot.</p></div>
  <Reveal className="world-contact-sheet"><div className="world-contact-sheet-heading"><span className="eyebrow">THE CONTACT SHEET</span><strong>Different worlds. <em>Same signature.</em></strong><span className="world-sheet-tip">PICK A FRAME TO EXPLORE ↗</span></div><div className="world-sheet-grid">{instagramWorlds.flatMap(channel=>channel.posts.slice(1).map(post=><a key={post.id} href={post.url} target="_blank" rel="noopener noreferrer" aria-label={`Open ${post.title} on Instagram (opens in a new tab)`}><Image src={post.image} alt={post.alt} fill sizes="(max-width: 600px) 28vw, 14vw"/><span className={`world-sheet-channel mood-${channel.id}`}>{channel.name}</span><div><span>{post.title}</span><ArrowUpRight size={17}/></div></a>))}</div></Reveal>
 </section>
}
